...
func send_player_state(mood: String, score: int, action: String) -> void:
	send_event({"type": "player_state", "data": {"mood": mood, "score": score, "action": action}})

func send_game_over(final_score: int) -> void:
	send_event({"type": "game_over", "data": {"score": final_score}})

func send_minigame_complete(score: int, skipped: bool) -> void:
	send_event({"type": "minigame_complete", "data": {"score": score, "skipped": skipped}})
